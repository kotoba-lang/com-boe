(ns boe.main
  "Kotodama WASM entrypoint for the Boe clean-room actor (L4) — Clojure port.
  ...
  facts (`boe.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"boe.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "boe-compat" :tier tier :entities entities} 200])
